...
from ramext.sql import pmpermit_sql as pm_sql
from . import *

WARN_PIC = Config.PMPERMIT_PIC or "https://telegra.ph/file/e62dcb31e02594af0fab4.png"
PM_WARNS = {}
PREV_REPLY_MESSAGE = {}
...
if PM_ON_OFF != "DISABLE":
    ...
    async def on_new_private_message(event):
        if not event.is_private:
            return
        cid = await client_id(event)
        ramtod, ram_mention = cid[0], cid[2]
        RAM_BOT = "**🔥 RAM-UBOT Prîvã†é Sêçürïty Prø†öçõl 🔥**\n\nThis is to inform you that {} is currently unavailable.\nThis is an automated message.\n\n{}\n\n**Please Choose Why You Are Here!!**".format(ram_mention, CSTM_PMP)
        if event.sender_id == ramtod:
            return
        if str(event.sender_id) in DEVLIST:
            return
        if Config.LOGGER_ID is None:
            await event.client.send_message(ramtod, "Please Set `LOGGER_ID` For Working Of Pm Permit")
            return
        message_text = event.message.raw_text
        chat_ids = event.sender_id
        if RAM_BOT == message_text:
            return
...
CmdHelp("pm_permit").add_command(
  ...
  "listapproved", None, "Sends the list of all users approved by RAM-UBOT"
).add_info(
  "PM SECURITY"
).add_warning(
  "✅ Harmless Module."
