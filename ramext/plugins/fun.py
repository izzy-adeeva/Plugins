...
async def slap(replied_user, event):
    ...
    ram_mention = cid[2]
    caption = temp.format(user1=ram_mention, user2=slapped, item=item, hits=hit, throws=throw)
    return caption


@ram_cmd(pattern=f"randi$")
async def rendi(e):
   txt = random.choice(RENDISTR)
   await eor(e, txt)
   
   
@ram_cmd(pattern=f"habuse$")
async def thenus(e):
   txt = random.choice(THANOS_STRINGS)
   await eor(e, txt)
   
   
@ram_cmd(pattern=f"fuk$")
async def tapatap(e):
   txt = random.choice(FUK_STRINGS)
   await eor(e, txt)
   
   
@ram_cmd(pattern=f"chu$")
async def chut(e):
   txt = random.choice(CHU_STRINGS)
   await eor(e, txt)
   
   
@ram_cmd(pattern=f"noob$")
async def nub(e):
   txt = random.choice(NOOBSTR)
   await eor(e, txt)


@ram_cmd(pattern=f"run$")
async def metoo(e):
    txt = random.choice(RUNSREACTS)
    await eor(e, txt)


@ram_cmd(pattern=f"gali$")
async def metoo(e):
    txt = random.choice(GAALI_STR)
    await eor(e, txt)


@ram_cmd(pattern=f"rape$")
async def metoo(e):
    txt = random.choice(RAPE_STRINGS)
    await eor(e, txt)


@ram_cmd(pattern=f"abuse$")
async def metoo(e):
    txt = random.choice(ABUSE_STRINGS)
    await eor(e, txt)


@ram_cmd(pattern=f"gey$")
async def metoo(e):
    txt = random.choice(GEY_STRINGS)
    await eor(e, txt)


@ram_cmd(pattern=f"piro$")
async def metoo(e):
    txt = random.choice(PRO_STRINGS)
    await eor(e, txt)


@ram_cmd(pattern=f"insult$")
async def metoo(e):
    txt = random.choice(INSULT_STRINGS)
    await eor(e, txt)


@ram_cmd(pattern=f"hiabuse$",)
async def metoo(e):
...
@ram_cmd(pattern="react$")
async def react_meme(react):
        await eor(react, random.choice(FACEREACTS))


@ram_cmd(pattern="clap(?:\s|$)([\s\S]*)")
