...
from ramext.sql import snip_sql as sq
from . import *
...
async def add_snip(event):
    ...
    if cht and not stri:
        ...
        return await eod(event, f"**ERROR** \nReply to a message with `{ii}snip <trigger>` to add snips...")
    if not cht:
        if stri:
            await event.client.send_message(lg_id, f"#NOTE \n\nAdded Note with  `#{trigger}`. Below message is the output. \n**DO NOT DELETE IT**")
            ...
            return await eod(event, f"Invalid Syntax. Check  `{ii}plinfo snips`  to get proper Syntax.")
...
async def lsnote(event):
    ...
    if len(all_snips) > 0:
        ...
        OUT_STR = f"No Snips. Start Saving using `{ii}snip`"
    if len(OUT_STR) > 4000:
