...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        await eod(event, f"`{ii}trt LanguageCode - message`  or  `{ii}trt LanguageCode as reply to a message.`\n\nTry `{ii}trc` to get all language codes")
        return
...
CmdHelp("google_asst").add_command(
  "voice", "<reply to a msg> <lang code>", "Sends the replied msg content in audio format."
    ...
    "trt", "<lang code> <reply to msg>", "Translates the replied message to desired language code. Type '.trc' to get all the language codes", f"trt en - hello | {ii}trt en <reply to msg>"
).add_command(
