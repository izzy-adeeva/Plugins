...
async def ff_mpeg_trim_cmd(event):
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
        ...
        if reply_message:
            ...
            ramevent = await eor(event, "`Saving the file...`")
            try:
                c_time = time.time()
                downloaded_file_name = await event.client.download_media(
                    ...
                    progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                        progress(d, t, ramevent, c_time, "trying to download")
                    ),
                )
...
async def ff_mpeg_trim_cmd(event):
    ...
    if len(cmt) == 3:
        ...
        if o is None:
            return await eod(ramevent, f"**Error : **`Can't complete the process`")
        try:
            c_time = time.time()
            await event.client.send_file(
                ...
                allow_cache=False,
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, ramevent, c_time, "trying to upload")
                ),
            )
            os.remove(o)
        except Exception as e:
            return await eod(ramevent, f"**Error : **`{e}`")
    elif len(cmt) == 2:
        ...
        await eod(ramevent, "RTFM")
        return
    end = datetime.datetime.now()
    ms = (end - start).seconds
    await eod(ramevent, f"`Completed Process in {ms} seconds`")


@ram_cmd(pattern="atrim(?:\s|$)([\s\S]*)")
async def ff_mpeg_trim_cmd(event):
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
        await eod(event, f"A media file needs to be download, and save to the following path:  `{FF_MPEG_DOWN_LOAD_MEDIA_PATH}`")
        return
    ...
    ms = (end - start).seconds
    await eod(ramevent, f"`Completed Process in {ms} seconds`", 3)


@ram_cmd(pattern="tclear$")
async def ff_mpeg_trim_cmd(event):
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
        await eod(event, "`There is no media saved in bot for triming`")
    else:
        os.remove(FF_MPEG_DOWN_LOAD_MEDIA_PATH)
        await eod(event, f"Deleted saved trimming media. You can save new media by `{ii}tsave`")
