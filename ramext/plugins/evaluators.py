...
@ram_cmd(pattern="exec(?:\s|$)([\s\S]*)")
async def _(event):
    cmd = "".join(event.text.split(maxsplit=1)[1:])
    if not cmd:
        return await eod(event, "`What should i execute?..`")
    ramevent = await eor(event, "`Executing.....`")
    process = await asyncio.create_subprocess_shell(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
    result = str(stdout.decode().strip()) + str(stderr.decode().strip())
    zyyuser = await event.client.get_me()
    if zyyuser.username:
        curruser = zyyuser.username
    else:
        curruser = "@RamUbot"
    uid = os.geteuid()
    if uid == 0:
...
async def _(event):
    if Config.USE_EVAL == "TRUE":
        ...
        ramevent = await eor(event, "`Running ...`")
        ...
        if len(final_output2) > 4092:
            await eor(ramevent, final_output, deflink=True, linktext=f"**•  Eval :** \n`{cmd}` \n\n**Pasted:** ")
        else:
            await eor(ramevent, final_output, deflink=True, linktext=f"{final_output2} \n\n**Also Pasted** ")
    else:
