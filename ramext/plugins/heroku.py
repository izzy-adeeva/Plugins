...
async def restart(event):
    if HEROKU_APP_NAME and HEROKU_API_KEY:
        ...
        await eor(event, f"✅ **Restarted RAM-UBOT** \n**Type** `{ii}ping` **after 1 minute to check if I am working !**")
    ...
    if exe == "get":
        ...
        if not value:
            ...
            try:
                ...
                return await event.edit(f"`{ii}set var <Var Name> <Value>`")
        if variable in config_list:
            return await event.edit(f"This is a SQL based variable. Do `{ii}svar {variable} {value}` to set this.")
        if variable in heroku_var:
            await event.edit(
        ...
        variable = xvar.upper()
        if variable in config_list:
            return await event.edit(f"This is a SQL based variable. Do `{ii}dvar {variable}` to delete it.")
        if variable in heroku_var:
...
CmdHelp("power").add_command(
  ...
  "shutdown", None, "Turns off RAM-UBOT. Userbot will stop working unless you manually turn it on."
).add_command(
