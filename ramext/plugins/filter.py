...
from ramext.sql.filter_sql import get_filter, add_filter, remove_filter, get_all_filters, remove_all_filters
from . import *
...
async def on_snip_save(event):
    ...
    if msg:
        ...
        await eod(event, f"Reply to a message with `{ii}filter keyword` to save the filter")


@ram_cmd(pattern="filters$")
async def on_snip_list(event):
    ...
    if len(all_snips) > 0:
        ...
        OUT_STR = f"No Filters. Start Saving using `{ii}filter`"
    if len(OUT_STR) > 4096:
