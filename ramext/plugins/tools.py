...
async def _(event):
    ...
    if not reply_message.media:
        ...
        return
    chat = "@DrWebBot"
    ramevent = await eor(event, " `Scanning This media..... wait👀`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await eod(ramevent, "`Please unblock `@DrWebBot `and try again`")
            return
...
async def _(event):
    ...
    try:
        ...
        await ramevent.edit(str(e))
        return
    end = datetime.datetime.now()
    ms = (end - start).seconds
    await ramevent.edit("Created BarCode in {} seconds🤓".format(ms))
    await asyncio.sleep(5)
    await ramevent.delete()
...
async def _(event):
    ...
    if input_str.startswith("#"):
        ...
        await eod(
            event, f"**Syntax : **`{ii}color <color_code>` example : `{ii}color #ff0000`"
        )


@ram_cmd(pattern="xkcd(?:\s|$)([\s\S]*)")
async def _(event):
    ramevent = await eor(event, "`processiong...`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
...
Year: {}""".format(
        ...
        await ramevent.edit(output_str, link_preview=True)
    else:
        await eod(ramevent, "xkcd n.{} not found!".format(xkcd_id))
