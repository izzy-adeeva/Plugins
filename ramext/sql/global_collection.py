...
class GlobalCollection(BASE):
    ...
    def __repr__(self):
        return "<sad Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


GlobalCollection.__table__.create(checkfirst=True)
...
def add_to_collectionlist(keywoard, contents):
    with sad_GLOBALCOLLECTION:
        keyword_items = GlobalCollection(keywoard, tuple(contents))
...
def rm_from_collectionlist(keywoard, contents):
    with sad_GLOBALCOLLECTION:
        keyword_items = SESSION.query(GlobalCollection).get(
...
def del_keyword_collectionlist(keywoard):
    with sad_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(GlobalCollection.keywoard)
            .filter(GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist():
    try:
        return SESSION.query(GlobalCollection).count()
    finally:
        SESSION.close()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(GlobalCollection).all()
        for x in all_groups:
