...
async def convert_to_image(event, client):
    iraa = await event.get_reply_message()
    if not (
            iraa.gif
            or iraa.audio
            or iraa.voice
    ...
    if not os.path.exists(downloaded_file_name):
        ...
        return
    if iraa and iraa.photo:
        iraa_final = downloaded_file_name
    elif iraa.sticker and iraa.sticker.mime_type == "application/x-tgsticker":
        ...
        os.remove(rpath)
        iraa_final = image_name20
    elif iraa.sticker and iraa.sticker.mime_type == "image/webp":
        ...
        if not os.path.exists(imgpath):
            ...
            return
        iraa_final = imgpath
    elif iraa.gif or iraa.video or iraa.video_note:
...
def tgs_to_gif(sticker_path: str, quality: int = 256) -> str:                  
    semx = os.path.join(dwlpath, "ramtgs.gif")
