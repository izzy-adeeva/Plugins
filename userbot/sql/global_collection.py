...
class ram_GlobalCollection(BASE):
    __tablename__ = "ram_globalcollection"
    ...
    def __repr__(self):
        return "<ram Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, ram_GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


ram_GlobalCollection.__table__.create(checkfirst=True)
...
def add_to_collectionlist(keywoard, contents):
    with sad_GLOBALCOLLECTION:
        keyword_items = ram_GlobalCollection(keywoard, tuple(contents))
...
def rm_from_collectionlist(keywoard, contents):
    with sad_GLOBALCOLLECTION:
        keyword_items = SESSION.query(ram_GlobalCollection).get(
...
def del_keyword_collectionlist(keywoard):
    with sad_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(ram_GlobalCollection.keywoard)
            .filter(ram_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(ram_GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist():
    try:
        return SESSION.query(ram_GlobalCollection).count()
    finally:
        SESSION.close()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(ram_GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(ram_GlobalCollection).all()
        for x in all_groups:
