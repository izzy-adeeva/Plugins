...
@ram_cmd(pattern="create (b|g|c) ([\s\S]*)")
async def _(event):
    type_of_group = event.pattern_match.group(1)
    group_name = event.pattern_match.group(2)
    TOD = await eor(event, "Creating wait sar.....")
    if type_of_group == "b":
        try:
        ...
        try:
            r = await event.client(
                functions.channels.CreateChannelRequest(
                    title=group_name,
                    about="Created By RAM-UBOT Extended",
        ...
        await TOD.edit(f"Read `{ii}plinfo create` to know how to use me")
