...
@ram_cmd(pattern="anime(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[7:]
    if query == "":
        return await eor(event, "Please give anime name to search on Anilist.")
    TOD = await eor(event, f"__Searching for__ `{query}` __on Anilist.__")
    qdb = rand_key()
    ANIME_DB[qdb] = query
    result = await get_anilist(qdb, 1)
    if len(result) == 1:
        return await TOD.edit(result[0])
...
async def _(event):
    ...
    try:
        ...
        await TOD.edit(finals_)
    if os.path.exists(pic):
        os.remove(pic)


@ram_cmd(pattern="character(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[11:]
    if query == "":
...
async def canon(event):
    ...
    nub = await eor(event, f"Searching Filler Episodes For `{TOD}`")
    hel_ = search_filler(TOD)
    if hel_ == {}:
        return await nub.edit(f"No filler found for `{TOD}`")
    list_ = list(hel_.keys())
    if len(list_) == 1:
        result = parse_filler(hel_.get(list_[0]))
        msg = ""
        msg += (
            f"<TOD2>Fillers for {list_[0]} :</TOD2>\n\n<b>Manga Canon Episodes :</b>\n"
        )
        ...
        return
    userbot = f"**📃 Filler Episode Lists :** \n\n"
    for i in list_:
        result = parse_filler(hel_.get(i))
        msg = ""
        msg += f"<TOD2>Fillers for {i} :</TOD2>\n\n<b>Manga Canon Episodes :</b>\n"
        msg += f'<code>{str(result.get("total_ep"))}</code>'
        msg += "\n\n<b>Mixed/Canon fillers :</b>\n"
        msg += f'<code>{str(result.get("mixed_ep"))}</code>'
        msg += "\n\n<b>Fillers :</b>\n"
        ...
        userbot += f"• [{i}]({paste})\n"
    await nub.edit(userbot)


@ram_cmd(pattern="airing(?:\s|$)([\s\S]*)")
async def _(event):
    query = event.text[8:]
    TOD = await eor(event, f"__Searching airing details for__ `{query}`")
    if query == "":
        return await eod(TOD, "Give anime name to seaech airing information.")
    vars_ = {"search": query}
    if query.isdigit():
...
async def _(event):
    query = event.text[9:]
    TOD = await eor(event, "Searching user's Anilist Stats...")
    if query == "":
        return await TOD.edit("No user found. Give anilist username.")
    qry = {"search": query}
    result = await get_user(qry)
    if len(result) == 1:
        return await eod(TOD, result[0])
    pic, msg = result
    try:
        await event.client.send_file(
            event.chat_id,
