...
async def mod(event):
    ...
    if not TOD:
        if event.is_reply:
            ...
            return
    uwu = await eor(event, f"Searching Mod for  `{TOD}` ...")
    troll = await event.client.inline_query("PremiumAppBot", f"{(deEmojify(TOD))}")
