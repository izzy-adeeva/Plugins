...
async def _(event):
    if not event.reply_to_msg_id:
        TOD = await eor(event, "Reply to any user message.")
        return
    ...
    if event.is_reply:
        ...
        if isinstance(data, bool):
            await TOD.edit("`I can't deep fry that!`")
            return
    if not event.is_reply:
        await TOD.edit("`Reply to an image or sticker to deep fry it!`")
        return
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await TOD.edit("Unblock @image_deepfrybot and try again")
            return
