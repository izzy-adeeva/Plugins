...
@ram_cmd(pattern="imdb(?:\s|$)([\s\S]*)")
async def _(event):
    reply_to = await reply_id(event)
    hel_ = await eor(event, "`Processing ...`")
    try:
        TOD = event.pattern_match.group(1)
        await hel_.edit("__Searching for__ `{}`".format(TOD))
        # Credits to catuserbot.
        # Ported to Hellbot and beautifications by @rampedo.
        movies = imdb.search_movie(TOD)
...
<b>Director :</b> <code>{mov_director}</code>"""
        ...
        paste = await telegraph_paste(f"IMDb Movie Result for “ {TOD} ”", telegraph_)
        ...
        await hel_.edit(f"__No movie found with name {TOD}.__")
