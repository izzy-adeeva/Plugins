...
async def device_info(request):
    ...
    if codename:
        ...
        await eor(request, f"`Usage: {ii}device <codename> / <model>`")
        return
    data = json.loads(
        get(
            "https://raw.githubusercontent.com/androidtrackers/"
...
@ram_cmd(pattern="codename(?: |)([\S]*)(?: |)([\s\S]*)")
async def codename_info(request):
    ...
    if brand and device:
        ...
        await eor(request, f"`Usage: {ii}codename <brand> <device>`")
        return
...
async def devices_specifications(request):
    ...
    if brand and device:
        ...
        await eor(request, f"`Usage: {ii}specs <brand> <device>`")
        return
    all_brands = (
        BeautifulSoup(
    ...
    try:
        device_page_url = [
            i.a["href"]
            for i in BeautifulSoup(str(devices), "lxml").findAll("TOD3")
...
async def twrp(request):
    ...
    if device:
        ...
        await eor(request, f"`Usage: {ii}twrp <codename>`")
        return
