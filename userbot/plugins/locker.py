...
@ram_cmd(pattern="lock(?:\s|$)([\s\S]*)")
async def _(event):
    text = event.text[6:]
    cid = await client_id(event)
    ram_mention = cid[2]
    TOD = await eor(event, f"Trying to lock `{text}`")
    if text == "":
        return await eod(TOD, "Need something to lock...")
    locker = lock(text)
    if not locker:
        return await eod(
            TOD,
    ...
    await event.client.send_file(
        event.chat_id,
        njirk,
        caption=f"{ram_mention} Locked `{text}` \n__Cause its Rest Time !!__",
    )
    await TOD.delete()
...
async def _(event):
    ...
    cid = await client_id(event)
    ram_mention = cid[2]
    TOD = await eor(event, f"Trying to unlock `{text}`")
    if text == "":
        return await eod(TOD, "Need something to unlock...")
    ...
    await event.client.send_file(
        event.chat_id, wahk, caption=f"**{ram_mention} unlocked** `{text}`"
    )
    await TOD.delete()
