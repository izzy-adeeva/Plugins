...
async def download(event):
    ...
    try:
        ...
        await userbot.edit("Committing to Github....")
        await git_commit(downloaded_file_name, path, branch, userbot)


async def git_commit(file_name, path, branch, userbot):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await userbot.edit("`File Already Exists`")
            create_file = False
    path = path
    file_name = file_name
    if create_file == True:
        ...
        try:
            repo.create_file(
                path,
                f"Uploaded file {file_name} by RAM-UBOT Extended",
                commit_data,
                branch=branch,
            )
            ...
            await eod(userbot, "Cannot Upload File")
    else:
        return await eod(userbot, "`Committed Suicide`")
...
CmdHelp("github").add_command(
    ...
    "commit ./userbot/plugins/example.py|master",
).add_command(
