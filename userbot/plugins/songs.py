...
async def _(event):
    ...
    rampedo, ram_mention = xyz[0], xyz[2]
    query = event.text[6:]
    max_results = 1
    if query == "":
        return await eod(event, "__Please give a song name to search.__")
    TOD = await eor(event, f"__Searching for__ `{query}`")
    ...
    try:
        ...
        return await eod(TOD, "`Max-downloads limit has been reached.`")
    except PostProcessingError:
        return await eod(TOD, "`There was an error during post processing.`")
    except UnavailableVideoError:
        return await eod(TOD, "`Media is not available in the requested format.`")
    except XAttrMetadataError as XAME:
        return await eod(TOD, f"`{XAME.code}: {XAME.msg}\n{XAME.reason}`")
    except ExtractorError:
        return await eod(TOD, "`There was an error during info extraction.`")
    except Exception as e:
        return await eod(TOD, f"{str(type(e)): {str(e)}}")
    c_time = time.time()
    await TOD.edit(
...
async def nope(iraa):
    TOD = iraa.text[8:]
    uwu = await eor(iraa, f"Searching lyrics for  `{TOD}` ...")
    if not TOD:
        if iraa.is_reply:
            ...
            return
    try:
        troll = await event.client.inline_query("iLyricsBot", f"{(deEmojify(TOD))}")
...
async def _(event):
    ...
    TOD = await eor(event, f"Searching song `{ram_}`")
    ...
    if somg:
        ...
        await TOD.edit("**ERROR 404 :** __NOT FOUND__")
...
async def _(event):
    ...
    chat = "@auddbot"
    TOD = await eor(event, "Trying to identify song...")
...
async def _(event):
    ...
    TOD = await eor(event, f"**Trying to download** `{text}` **from Spotify...**")
    async with event.client.conversation(chat) as conv:
        try:
            first = await conv.send_message("/start")
            second = await conv.get_response()
            ...
            if somg:
                ...
                return await eod(TOD, "**ERROR !!** __404 : NOT FOUND__")
            fourth = await conv.get_response()
            fifth = await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
        except YouBlockedUserError:
            return await eod(TOD, f"Please unblock {chat} to use Spotify module.")
        except Exception as e:
            return await eod(TOD, f"**ERROR !!** \n\n{e}")
