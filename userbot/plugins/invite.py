...
async def get_users(event):
    ...
    TOD = await eor(event, f"__Inviting members from__ {hel_}")
    if hell_chat in restricted:
        await TOD.edit("You can't Invite Members from there.")
    ...
    if event.is_private:
        return await TOD.edit("`Sorry, Cant add users here`")
    s = 0
    f = 0
    error = "None"
    await TOD.edit("**INVITING USERS !!**")
...
@ram_cmd(pattern="add(?:\s|$)([\s\S]*)")
async def _(event):
    if "addsudo" in event.raw_text.lower() or "addblacklist" in event.raw_text.lower():
        return
    to_add_users = event.pattern_match.group(1)
    if event.is_private:
        await eod(event, f"Use `{ii}add` users to a chat, not to a Private Message")
    else:
