...
async def google(event):
    ...
    res = f"""<TOD3><b><i>Google Search Query :</b></i> <u>{input_str}</u></TOD3>
...
async def img(event):
    ...
    if not sim:
        return await eod(event, "`Give something to search...`")
    TOD = await eor(event, f"Searching for `{sim}`...")
    if ";" in sim:
        try:
            lim = int(sim.split(";")[1])
            sim = sim.split(";")[0]
...
async def _(event):
    ...
    link = alls["href"]
    text = alls.text
    await TOD.edit(f"**Possible Results :** [{text}](google.com{link})")
...
async def gps(event):
    ...
    if geoloc:
        ...
        await eod(TOD, "I coudn't find it😫")
