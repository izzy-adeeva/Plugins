...
async def restart(event):
    if HEROKU_APP_NAME and HEROKU_API_KEY:
        ...
        await eor(
            event,
            f"✅ **Restarted Dynos** \n**Type** `{ii}ping` **after 1 minute to check if I am working !**",
        )
...
async def re(TOD):
    event = await eor(TOD, "Restarting RAM-UBOT Extended ...")
    try:
        await restart(event)
...
async def rel(event):
    await eor(event, "Reloading RAM-UBOT Extended... Wait for few seconds...")
    await reload_userbot()


@ram_cmd(pattern="shutdown$")
async def down(TOD):
    event = await eor(TOD, "`Turing Off RAM-UBOT Extended...`")
    await asyncio.sleep(2)
    await event.edit(
        "**[ ⚠️ ]** \n**RAM-UBOT Extended is now turned off. Manually turn it on to start again.**"
    )
...
@ram_cmd(pattern="svar(?:\s|$)([\s\S]*)")
async def sett(event):
    hel_ = event.pattern_match.group(1)
    var_ = hel_.split(" ")[0].upper()
    val_ = hel_.split(" ")[1:]
    valu = " ".join(val_)
    TOD = await eor(event, f"**Setting variable** `{var_}` **as** `{valu}`")
    if var_ == "":
    ...
    if var_ not in config_list:
        return await TOD.edit(
            f"__There isn't any variable named__ `{var_}`. __Check spelling or get full list by__ `{ii}vars`"
        )
    try:
        addgvar(var_, valu)
    except Exception as e:
        return await TOD.edit(f"**ERROR !!** \n\n`{e}`")
    await TOD.edit(f"**Variable** `{var_}` **successfully added with value** `{valu}`")


@ram_cmd(pattern="gvar(?:\s|$)([\s\S]*)")
async def gett(event):
    var_ = event.pattern_match.group(1).upper()
    TOD = await eor(event, f"**Getting variable** `{var_}`")
    if var_ == "":
        return await TOD.edit(
    ...
    if var_ not in config_list:
        return await TOD.edit(
            f"__There isn't any variable named__ `{var_}`. __Check spelling or get full list by `{ii}vars`"
        )
    try:
        sql_v = gvarstat(var_)
...
async def dell(event):
    var_ = event.pattern_match.group(1).upper()
    TOD = await eor(event, f"**Deleting Variable** `{var_}`")
    if var_ == "":
        return await TOD.edit(
    ...
    if var_ not in config_list:
        return await TOD.edit(
            f"__There isn't any variable named__ `{var_}`. Check spelling or get full list by `{ii}vars`"
        )
    try:
        delgvar(var_)
    except Exception as e:
        return await TOD.edit(f"**ERROR !!** \n\n`{e}`")
    await TOD.edit(f"**Deleted Variable** `{var_}`")


@ram_cmd(pattern="(set|get|del) var(?: |$)(.*)(?: |$)([\s\S]*)")
async def variable(TOD):
    if Config.HEROKU_APP_NAME is not None:
        app = Heroku.app(Config.HEROKU_APP_NAME)
    else:
        return await eor(TOD, "`[HEROKU]:" "\nPlease setup your` **HEROKU_APP_NAME**")
    exe = TOD.pattern_match.group(1)
    heroku_var = app.config()
    if exe == "get":
        event = await eor(TOD, "Getting Variable Info...")
        cap = "Check the logs."
        capn = "Saved in LOGGER_ID !!"
        try:
            xvar = TOD.pattern_match.group(2).split()[0]
            variable = xvar.upper()
            if variable in config_list:
                return await event.edit(
                    f"This is a SQL based variable. Do `{ii}gvar {variable}` to get variable info."
                )
            if variable in ("STRING_SESSION", "BOT_TOKEN", "HEROKU_API_KEY"):
                if Config.ABUSE == "ON":
                    await event.client.send_file(TOD.chat_id, tod, caption=cap)
                    await event.delete()
        ...
        event = await eor(TOD, "Setting Heroku Variable...")
        xvar = TOD.pattern_match.group(2)
        if not xvar:
            return await event.edit(f"`{ii}set var <Var Name> <Value>`")
        variable = xvar.upper()
        value = TOD.pattern_match.group(3)
        if not value:
            variable = variable.split()[0]
            try:
                value = TOD.pattern_match.group(2).split()[1]
            except IndexError:
                return await event.edit(f"`{ii}set var <Var Name> <Value>`")
        if variable in config_list:
            return await event.edit(
                f"This is a SQL based variable. Do `{ii}svar {variable} {value}` to set this."
            )
        if variable in heroku_var:
        ...
        event = await eor(TOD, "Getting info to delete Variable")
        try:
            xvar = TOD.pattern_match.group(2).split()[0]
        except IndexError:
        ...
        if variable in config_list:
            return await event.edit(
                f"This is a SQL based variable. Do `{ii}dvar {variable}` to delete it."
            )
        if variable in heroku_var:
...
async def dyno_usage(TOD):
    event = await eor(TOD, "`Processing...`")
    useragent = (
...
CmdHelp("power").add_command(
    ...
    "shutdown",
    None,
    "Turns off RAM-UBOT Extended. Userbot will stop working unless you manually turn it on.",
).add_command(
    "svar",
