...
async def ram_info(rambot_info):
    _version = "extended v.01"
    _release = "march 2022"
    _branch = "master"
    _author = "RAM ETOD"
    _auturl = "https://github.com/hitokizzy"
...
async def upstream(event):
    ...
    if changelog == "" and not force_update:
        ...
        output_ = f"**Your Bot Version :** `{ram_ver}` \n**Owner :** {ram_mention} \n\n**Official RAM-UBOT Version :** `{_version}` \n**Release Date :** `{_release}` \n**Official Repo Branch :** `{_branch}` \n**Update By :** [{_author}]({_auturl})"
        if str(_version) not in str(ram_ver):
            output_ += f"\n\n**Do** `{ii}update build` **to update your RAM to latest version.**"
        await event.edit(output_)
        return repo.__del__()
    if conf == "" and not force_update:
        await print_changelogs(event, ac_br, changelog)
        await event.delete()
        return await event.respond(f"🌚 Do `{ii}update build` to update your RAM !!")

    if force_update:
        await event.edit("`Force-Updating RAM-UBOT EXTENDED. Please wait...`")
    if conf == "now":
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        await event.edit(
            f"**Your RAM-UBOT EXTENDED Is UpToDate**\n\n**Version :**  __{ram_ver}__\n**Oɯɳҽɾ :**  {ram_mention}"
        )
    else:
        await event.edit(
...
async def upstream(event):
    ...
    off_repo = "https://github.com/hitokizzy/RAM_UBOT_EXT"
    os.chdir("/app")
    ...
    ups_rem = repo.remote("upstream")
    ups_rem.fetch(ac_br)
    _version, _release, _branch, _author, _auturl = await hell_info(ram_info)
    await event.edit(
        f"<b><i>RAM-UBOT EXTENDED Docker Build In Progress !!</b></i> \n\n<b><i><u>Update Information :</b></i></u> \n<b>• Branch :</b> {_branch} \n<b>• Release Date :</b> {_release} \n<b>• Version :</b> {_version} \n<b>• Author :</b> <a href='{_auturl}'>{_author}</a>",
        link_preview=False,
        parse_mode="HTML",
...
CmdHelp("update").add_command(
    ...
    "update now",
    None,
    "Soft-Update Your RAM-UBOT Extended. Basically if you restart dyno it will go back to previous deploy.",
).add_command(
    "update build",
    None,
    "Hard-Update Your RAM-UBOT Extended. This won't take you back to your previous deploy. This will be triggered even if there is no changelog.",
).add_info(
    "RAM-UBOT Extended Updater."
).add_warning(
    "✅ Harmless Module."
).add()
