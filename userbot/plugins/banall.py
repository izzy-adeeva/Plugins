...
async def _(event):
    ...
    TOD = await eor(event, "**Bleck Magik Started...**")
    ...
    async for user in event.client.iter_participants(event.chat_id):
        ...
        try:
            ...
            LOGS.info(str(e))
            await asyncio.sleep(0.5)
    await TOD.edit("**Bleck Magik Done...**")
...
async def _(event):
    ...
    TOD = await eor(event, "**Bleck Magik Begins..**")
    admins = await event.client.get_participants(
        event.chat_id, filter=ChannelParticipantsAdmins
    )
    admins_id = [i.id for i in admins]
    ...
    async for user in event.client.iter_participants(event.chat_id):
        ...
        try:
            ...
            LOGS.info(str(e))
            await asyncio.sleep(0.5)
    await TOD.edit("**Bleck Magik Completed...**")
    await event.client.send_message(
        Config.LOGGER_ID,
...
async def _(event):
    ...
    q = 0
    r = 0
    TOD = await eor(event, "Searching Participant Lists.")
    async for i in event.client.iter_participants(event.chat_id):
        ...
        if isinstance(i.status, UserStatusEmpty):
            ...
            if "y" in input_str:
                ...
                if not status:
                    await eod(TOD, "I need admin priveleges to perform this action!")
                    e.append(str(e))
                    break
                else:
                    c = c + 1
...
Bots: {}
None: {}"""
        await TOD.edit(required_string.format(c, p, d, y, m, w, o, q, r, b, n))
