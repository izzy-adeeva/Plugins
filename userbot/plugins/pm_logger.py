...
@ram_cmd(pattern="save(?:\s|$)([\s\S]*)")
async def log(event):
    if f"{ii}savewelcome" in event.text:
        return
...
async def monito_p_m_s(event):
    ...
    if not sender.bot:
        ...
        if lg_id is not None:
            if chat.id not in NO_PM_LOG_USERS and chat.id != rampedo:
                try:
