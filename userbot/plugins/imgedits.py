...
async def fun(event):
    path = "omk"
    TOD = await eor(event, "Editing In Progress...")
...
async def fun(event):
    path = "omk"
    TOD = await eor(event, "Warping In Progress...")
    reply = await event.get_reply_message()
    lol = await event.client.download_media(reply.media, path)
...
async def fun(event):
    path = "omk"
    TOD = await eor(event, "Distortion In Progress...")
    reply = await event.get_reply_message()
    lol = await event.client.download_media(reply.media, path)
    file_name = "dist.jpg"
    hehe = path + "/" + file_name
...
CmdHelp("imgedits").add_command(
    "feye",
    "<reply to a img/stcr>",
    "Edits the replied image or sticker to a 3-D Box like image.",
).add_command(
    "warp",
    "<reply to a img/stcr>",
