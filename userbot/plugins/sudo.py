...
async def add(event):
    suu = event.text[9:]
    if f"{ii}add " in event.text:
        return
    ok = await eor(event, "**🚀 Adding Sudo User...**")
    rply = await event.get_reply_message()
    if not suu and not rply:
        return await ok.edit(
            "Either reply to a user or give user ids to add them to your sudo users list."
        )
    if suu:
    ...
    await ok.edit(
        f"**Successfully Added New Sudo User.** \n\n__Restart your bot to apply changes. Do__ `{ii}reload`"
    )


@ram_cmd(pattern="rmsudo(?:\s|$)([\s\S]*)")
async def _(event):
    suu = event.text[8:]
    ...
    if not suu and not rply:
        return await ok.edit(
            "Either reply to a user or give user ids to remove them from your sudo users list."
        )
    if suu:
        if not suu.isnumeric():
