...
async def _(event):
    ...
    if event.reply_to_msg_id and input_str == "":
        ...
        try:
            ...
            await TOD.edit("`Removing background of this media`")
            file_name = toimage(file_name)
            response = ReTrieveFile(file_name)
            os.remove(file_name)
    elif input_str:
        TOD = await eor(event, "`Removing Background of this media`")
        response = ReTrieveURL(input_str)
    else:
        await eod(
            event,
            f"Reply to a image/sticker with `{ii}rmbg` or give image link to remove background.",
        )
        return
    contentType = response.headers.get("content-type")
    remove_bg_image = "RamBot.png"
    if "image" in contentType:
        with open("RamBot.png", "wb") as removed_bg_file:
            removed_bg_file.write(response.content)
    else:
        await eod(TOD, f"`{response.content.decode('UTF-8')}`")
        return
    if flag and flag == "-s":
        file = tosticker(remove_bg_image, filename="RamBot.webp")
