...
async def _(event):
    ...
    TOD = await eor(event, "Converting...")
    if event.pattern_match.group(1):
        quality = event.pattern_match.group(1)
    else:
        quality = 512
    rply = await event.get_reply_message()
    TOD = await event.client.download_media(rply.media)
    gifs = tgs_to_gif(TOD, quality)
...
async def _(TOD):
    reply_to_id = TOD.message.id
    if TOD.reply_to_msg_id:
        reply_to_id = TOD.reply_to_msg_id
    event = await eor(TOD, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await TOD.client.download_media(
