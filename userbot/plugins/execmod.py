...
async def pipcheck(pip):
    ...
    if pipmodule:
        ...
        await piip.edit(f"`Use {ii}plinfo execmod to see an example`")


@ram_cmd(pattern="suicide$")
async def _(event):
...
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**RAM-UBOT Extended's Environment Module :**\n\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await event.client.send_file(
...
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**RAM-UBOT Extended's Server Speed Calculated :**\n\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await event.client.send_file(
...
CmdHelp("execmod").add_command(
    ...
    "env", None, "Shows Environment veriables of your RamBot"
).add_command(
    "speed", None, "Shows server speed of your RamBot"
).add_info(
    "Exec Modules."
).add_warning(
    "✅ Harmless Module."
