...
# Ported from Pyrogram to Telethon by @rampedo for RAM_UBOT Extended
...
async def glitch_(event):
    TOD = await eor(event, "`Trying to glitch this ...`")
    replied = await event.get_reply_message()
    inp = event.text[8:]
    if not (
        replied and (replied.photo or replied.sticker or replied.video or replied.gif)
    ):
        return await TOD.edit("```Media not found...```")
    if inp != "":
        if not inp.isdigit():
            return await eod(TOD, "**Invalid Input !!** \n\nPlease enter digits only.")
        input_ = int(inp)
        if not 0 < input_ < 9:
            return await eod(TOD, "**Invalid Range !!** \n\n**Valid Range** - 1 to 8")
        args = input_
    else:
        args = 2
    if not os.path.isdir(Config.TMP_DOWNLOAD_DIRECTORY):
        os.makedirs(Config.TMP_DOWNLOAD_DIRECTORY)
    await TOD.edit("`Glitchingggggg`")
    ...
    if dls.endswith(".tgs"):
        ...
        if not os.path.lexists(file_1):
            await eod(TOD, "```Sticker not found...```")
