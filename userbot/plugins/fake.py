...
async def _(event):
    ...
    TOD = await eor(event, "`Creating a fake face...`")
    if response.status_code == 200:
        with open("HELLBOT.jpg", "wb") as f:
            f.write(response.content)
    else:
        return await eod(TOD, "Failed to create Fake Face! Try again later.")
...
async def gbun(event):
    ...
    TOD = await eor(event, "** Nikal Lawde❗️⚜️☠️**")
    asyncio.sleep(3.5)
    ...
    if event.reply_to_msg_id:
        ...
        if idd == 1432756163:
            await TOD.edit(
                "`Wait a second, This is my master!`\n**How dare you threaten to ban my master nigger!**\n\n__Your account has been hacked! Pay 69$ to my master__ [HellBoy](https://t.me/rampedo) __to release your account__😏",
                link_preview=False,
            )
        else:
            jnl = (
