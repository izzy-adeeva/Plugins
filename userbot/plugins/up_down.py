...
@ram_cmd(pattern="upld_dir(?:\s|$)([\s\S]*)")
async def uploadir(event):
    input_str = event.pattern_match.group(1)
    if os.path.exists(input_str):
        TOD = await eor(event, "Downloading Using Userbot Server....")
        lst_of_files = []
        ...
        for single_file in lst_of_files:
            if os.path.exists(single_file):
                ...
                uploaded = uploaded + 1
        await TOD.edit("Uploaded {} files successfully !!".format(uploaded))
    else:
        await TOD.edit("404: Directory Not Found")
...
async def upload(event):
    ...
    if input_str == "config.env":
        await event.client.send_file(event.chat_id, tod, caption=cap)
        await event.delete()
        return
    if os.path.exists(input_str):
        c_time = time.time()
        ...
        await TOD.edit("Uploaded successfully !!")
    else:
        await TOD.edit("404: File Not Found")
    ...
    if os.path.exists(file_name):
        ...
        try:
            if supports_streaming:
                ...
                await TOD.edit("TBD: Not (yet) Implemented")
                return
            os.remove(thumb)
            await TOD.edit("Uploaded successfully !!")
        except FileNotFoundError as err:
            await TOD.edit(str(err))
    else:
        await TOD.edit("404: File Not Found")
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            downloaded_file_name = await event.client.download_media(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, TOD, c_time, "trying to download")
                ),
            )
        except Exception as e:  # pylint:disable=C0103,W0703
            await TOD.edit(str(e))
        else:
        ...
        while not downloader.isFinished():
            ...
            try:
                ...
                if round(diff % 10.00) == 0 and current_message != display_message:
                    await TOD.edit(current_message)
        ...
        if downloader.isSuccessful():
            ...
            await TOD.edit("Incorrect URL\n {}".format(input_str))
    else:
        await TOD.edit("Reply to a message to download to my local server.")
