...
@ram_cmd(pattern="thug$")
async def _(event):
    if not event.reply_to_msg_id:
        return await eod(event, "Reply to a image...")
    TOD = await eor(event, "`Converting To thug Image..`")
    await event.get_reply_message()
    img = await convert_to_image(event, event.client)
    imagePath = img
    maskPath = "./userbot/resources/pics/mask (1).png"
    cascPath = "./userbot/resources/xmls/haarcascade_frontalface_default.xml"
...
async def _(event):
    TOD = await eor(event, "`Trigggggggggerrr`")
...
@ram_cmd(pattern="pix$")
async def _(event):
    TOD = await eor(event, "`Pixing This Image.`")
    owo = await event.get_reply_message()
    img = await convert_to_image(event, event.client)
    url_s = upload_file(img)
    imglink = f"https://telegra.ph{url_s[0]}"
...
async def _(event):
    TOD = await eor(event, "`Making a youthuub comment...`")
    owo = await event.get_reply_message()
