...
userbot = gvarstat("STICKER_PACKNAME")
...
async def kang(event):
    ...
    un_ = user.username if user.username else rampedo
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            TOD = await eor(event, f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
    ...
    if photo:
        ...
        packnick = (
            f"{userbot} Vol.{pack}"
            if userbot
            else f"{un}'s RAM-UBOT Extended Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
        ...
        if (
            ...
            await TOD.edit("`Preparing a new pack....`")
            async with event.client.conversation("Stickers") as conv:
                await conv.send_message(cmd)
                await conv.get_response()
                # Ensure user doesn't get spamming notifications
...
async def get_pack_info(event):
    ...
    try:
        ...
        TOD = await eor(event, "`Fetching details of the sticker pack, please wait..`")
    except BaseException:
        await eod(event, "`This is not a sticker. Reply to a sticker.`")
        return

    if not isinstance(stickerset_attr, DocumentAttributeSticker):
        await TOD.edit("`This is not a sticker. Reply to a sticker.`")
        return
...
async def _(event):
    ...
    TOD = await eor(event, "🥴 `Deleting sticker...`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await TOD.edit("Please unblock @Stickers and try again")
            return
...
async def _(event):
    ...
    chat = "@Stickers"
    TOD = await eor(event, "📝 `Editing sticker emoji...`")
    if hel_ == "":
        await TOD.edit("**🤧 Nashe me hai kya lawde**")
    else:
        async with event.client.conversation(chat) as conv:
            try:
                response = conv.wait_event(
                ...
                await TOD.edit("Please unblock @Stickers and try again")
                return
            await TOD.edit(f"{response.message}")
...
async def _(event):
    ...
    if TOD == "":
        pname = f"{un}'s RAM-UBOT Extended Pack"
    else:
        pname = TOD
