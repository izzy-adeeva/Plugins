...
async def _(event):
    ...
    if not reply_message.media:
        ...
        return
    chat = "@DrWebBot"
    anu = await eor(event, " `Scanning This media..... wait👀`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await eod(anu, "`Please unblock `@DrWebBot `and try again`")
            return
        if response.text.startswith("Forward"):
            await eod(
                anu, "Can you kindly disable your forward privacy settings for good?"
            )
        else:
            if response.text.startswith("Select"):
                await eod(anu, "`Please go to` @DrWebBot `and select your language.`")
            else:
                await anu.edit(
...
@ram_cmd(pattern="barcode(?:\s|$)([\s\S]*)")
async def _(event):
    anu = await eor(event, "...")
    start = datetime.datetime.now()
    input_str = event.pattern_match.group(1)
    message = f"SYNTAX: `{ii}barcode <long text to include>`"
    ...
    try:
        ...
        await anu.edit(str(e))
        return
    end = datetime.datetime.now()
    ms = (end - start).seconds
    await anu.edit("Created BarCode in {} seconds🤓".format(ms))
    await asyncio.sleep(5)
    await anu.delete()


@ram_cmd(pattern="makeqr(?: |$)([\s\S]*)")
async def make_qr(event):
    input_str = event.pattern_match.group(1)
    message = f"SYNTAX: `{ii}makeqr <long text to include>`"
    reply_msg_id = None
    if input_str:
        message = input_str
    elif event.reply_to_msg_id:
...
async def _(event):
    ...
    TOD = await eor(event, "Processing...")
    ...
    if len(input_sgra) == 3:
        ...
        await TOD.edit(str(a))
    else:
        await eod(TOD, f"SYNTAX: {ii}calendar YYYY.MM.DD")
...
async def _(event):
    ...
    if len(input_sgra) == 3:
        ...
        await edit_or_reply(
            event,
            f"**Syntax:**\n{ii}currency amount from to\n**Example:**\n`{ii}currency 10 usd inr`",
        )


@ram_cmd(pattern="currencies$")
...
async def _(event):
    ...
    if input_str.startswith("#"):
        ...
        await eod(
            event,
            f"**Syntax : **`{ii}color <color_code>` example : `{ii}color #ff0000`",
        )


@ram_cmd(pattern="xkcd(?:\s|$)([\s\S]*)")
async def _(event):
    anu = await eor(event, "`processiong...`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
...
Year: {}""".format(
        ...
        await anu.edit(output_str, link_preview=True)
    else:
        await eod(anu, "xkcd n.{} not found!".format(xkcd_id))


@ram_cmd(pattern="dns(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.pattern_match.group(1)
