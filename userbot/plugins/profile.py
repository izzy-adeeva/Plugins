...
async def _(event):
    ...
    if user.first_name.startswith(OFFLINE_TAG):
        ...
        return
    TOD = await eor(event, "**Changing Profile to Offline...**")
    ...
    if photo:
        ...
        try:
            ...
            await eod(TOD, "**Changed profile to OffLine.**")
    try:
...
async def _(event):
    ...
    if photo:
        ...
        try:
            ...
            await eod(TOD, "**Changed profile to Online.**")
    try:
        os.system("rm -fr donottouch.jpg")
    except Exception as e:
        LOGS.warn(str(e))
...
async def _(event):
    ...
    TOD = await eor(event, "Downloading Profile Picture to my local ...")
    if not os.path.isdir(Config.TMP_DOWNLOAD_DIRECTORY):
        os.makedirs(Config.TMP_DOWNLOAD_DIRECTORY)
    photo = None
    try:
        ...
        if photo:
            await TOD.edit("Uploading profile picture...")
            file = await event.client.upload_file(photo)
            try:
                await event.client(functions.photos.UploadProfilePhotoRequest(file))
            except Exception as e:
...
@ram_cmd(pattern="username(?:\s|$)([\s\S]*)")
...
async def count(event):
    ...
    await TOD.edit(result, parse_mode="HTML")
