...
@ram_cmd(pattern="recognize(?:\s|$)([\s\S]*)")
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    TOD = await eor(event, "recognizeing this media")
...
<b>⚡ <a href='https://t.me/its_userbot'>From DataBase of RamBot</a> ⚡ </b>
""".format(
        user_id,
        user_id,
        first_name,
...
async def info(event):
    TOD = await eor(event, "`Analysing the chat...`")
    ...
    try:
        ...
        print("Exception:", e)
        await eod(TOD, "`An unexpected error has occurred.`")
    return
...
@ram_cmd(pattern="id$")
async def _(event):
    TOD = await eor(event, "Fetching Ids...")
    if event.reply_to_msg_id:
        ...
        await TOD.edit("🔸 **Current Chat ID:** `{}`".format(str(event.chat_id)))
