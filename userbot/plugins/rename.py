...
@ram_cmd(pattern="rename(?:\s|$)([\s\S]*)")
async def _(event):
    input_str = event.text[8:]
    if input_str == "":
        return await eod(event, "Give a new file name..")
    TOD = await eor(event, f"Renaming it to `{input_str}`")
...
async def _(event):
    ...
    TOD = await eor(event, f"Renaming to `{input_str}`")
    if not os.path.isdir(Config.TMP_DOWNLOAD_DIRECTORY):
        os.makedirs(Config.TMP_DOWNLOAD_DIRECTORY)
    if event.reply_to_msg_id:
        start = datetime.datetime.now()
        ...
        await TOD.edit(f"**Syntax Wrong !!** \n\n• `{ii}rnupload new file name`")


@ram_cmd(pattern="rnsupload(?:\s|$)([\s\S]*)")
async def _(event):
    TOD = await eor(event, "Rename & Upload as streamable format is in progress...")
    ...
    if event.reply_to_msg_id:
        ...
        if os.path.exists(downloaded_file_name):
            ...
            await eod(TOD, "File Not Found {}".format(input_str))
    else:
        await TOD.edit(
