...
if PM_ON_OFF != "DISABLE":
    ...
    @ramm.on(events.NewMessage(outgoing=True))
    async def _(event):
        ...
        if not pm_sql.is_approved(event.chat_id):
            if not event.chat_id in PM_WARNS:
                ...
                x = await event.client.send_message(
                    event.chat_id, "**Auto Approved because outgoing message.**"
                )
                await asyncio.sleep(4)
                await x.delete()

    @ramm.on(admin_cmd(pattern="(a|approve|allow)$"))
    ...
    @ramm.on(admin_cmd(pattern="(da|disapprove|disallow)$"))
    ...
    @ramm.on(admin_cmd(pattern="listapproved$"))
    async def approve_p_m(event):
    ...
    async def on_new_private_message(event):
        if not event.is_private:
            return
        cid = await client_id(event)
        rampedo, ram_mention = cid[0], cid[2]
        RAM_FIRST = "**🔥 RAM-UBOT Extended Prîvã†é Sêçürïty Prø†öçõl 🔥**\n\nThis is to inform you that {} is currently unavailable.\nThis is an automated message.\n\n{}\n\n**Please Choose Why You Are Here!!**".format(
            ram_mention, CSTM_PMP
        )
        ...
        if Config.LOGGER_ID is None:
            await event.client.send_message(
                rampedo, "Please Set `LOGGER_ID` For Working Of Pm Permit"
            )
            return
        message_text = event.message.raw_text
        chat_ids = event.sender_id
        if RAM_FIRST == message_text:
            return
...
CmdHelp("pm_permit").add_command(
    ...
    "listapproved", None, "Sends the list of all users approved by RAM-UBOT Extended"
).add_info(
    "PM SECURITY"
).add_warning(
    "✅ Harmless Module."
