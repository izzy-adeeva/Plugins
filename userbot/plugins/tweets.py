...
async def nope(iraa):
    ...
    if not TOD:
        if iraa.is_reply:
            ...
            return
    tweeter = await event.client.inline_query("TwitterStatusBot", f"{(deEmojify(TOD))}")
