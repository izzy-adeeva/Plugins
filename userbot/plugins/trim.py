...
async def ff_mpeg_trim_cmd(event):
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
        ...
        if reply_message:
            ...
            anu = await eor(event, "`Saving the file...`")
            try:
                c_time = time.time()
                downloaded_file_name = await event.client.download_media(
                    ...
                    progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                        progress(d, t, anu, c_time, "trying to download")
                    ),
                )
...
async def ff_mpeg_trim_cmd(event):
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
        ...
        return
    anu = await eor(event, "`Triming the media......`")
    ...
    if len(cmt) == 3:
        ...
        if o is None:
            return await eod(anu, f"**Error : **`Can't complete the process`")
        try:
            c_time = time.time()
            await event.client.send_file(
                ...
                allow_cache=False,
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, anu, c_time, "trying to upload")
                ),
            )
            os.remove(o)
        except Exception as e:
            return await eod(anu, f"**Error : **`{e}`")
    elif len(cmt) == 2:
    ...
    ms = (end - start).seconds
    await eod(anu, f"`Completed Process in {ms} seconds`")
...
async def ff_mpeg_trim_cmd(event):
    ...
    ms = (end - start).seconds
    await eod(anu, f"`Completed Process in {ms} seconds`", 3)
...
async def ff_mpeg_trim_cmd(event):
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
        ...
        await eod(
            event,
            f"Deleted saved trimming media. You can save new media by `{ii}tsave`",
        )
