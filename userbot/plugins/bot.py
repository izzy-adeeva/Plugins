...
async def leave(e):
    await e.edit("😪 **KThnxBye** See u all in TOD!!")
    time.sleep(1)
    if "-" in str(e.chat_id):
...
async def lst(event):
    TOD = await eor(event, "Getting configs list...")
...
async def _(event):
    ...
    ttl = 0
    message = f"SYNTAX: `{ii}schd <time_in_seconds> - <message to send>`"
    if input_str:
        await event.delete()
        if "-" in input_str:
...
async def _(event):
    ...
    try:
        ...
        await eod(f"**Invalid Syntax !!**\n\n`{ii}dm <Username or UserID> <message>`")
