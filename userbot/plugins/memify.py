...
async def _(event):
    ...
    TOD = await _reply.download_media()
    if TOD.endswith((".tgs")):
        await hel_.edit("OwO animated sticker...")
        cmd = ["lottie_convert.py", TOD, "pic.png"]
        file = "pic.png"
        ...
        stderr.decode().strip()
        stdout.decode().strip()
    elif TOD.endswith((".webp", ".png")):
        pics = Image.open(TOD)
        pics.save("pic.png", format="PNG", optimize=True)
        file = "pic.png"
    else:
        img = cv2.VideoCapture(TOD)
        tal, semx = img.read()
...
async def nope(event):
    ...
    troll = await event.client.inline_query("DogeStickerBot", f"{(deEmojify(TOD))}")
    if troll:
        await event.delete()
...
async def nope(event):
    ...
    troll = await event.client.inline_query("GooglaxBot", f"{(deEmojify(TOD))}")
    if troll:
        await event.delete()
        hel_ = await troll[0].click(Config.LOGGER_ID)
        if hel_:
...
async def nope(event):
    ...
    troll = await event.client.inline_query("honka_says_bot", f"{(deEmojify(TOD))}.")
    if troll:
        await event.delete()
        hel_ = await troll[0].click(Config.LOGGER_ID)
        if hel_:
...
async def nope(event):
    ...
    troll = await event.client.inline_query("stickerizerbot", f"#12{(deEmojify(TOD))}")
    if troll:
        await event.delete()
        hel_ = await troll[0].click(Config.LOGGER_ID)
        if hel_:
