...
async def spammer(event):
    ...
    TOD = await eor(event, f"Spamming {counter} times...")
...
async def bigspam(event):
    msg_ = event.text[9:]
    userbot_count = int(msg_.split(" ")[0])
    reply_msg = await event.get_reply_message()
    if reply_msg:
        hell_spam = reply_msg
    else:
        hell_spam = msg_.replace(str(userbot_count), "")
    for i in range(userbot_count):
...
# Special Break Spam Module For RamBot Made By Chirag Bhargava.
# Team RamBot
...
async def tiny_pic_spam(event):
    try:
        ...
        return await event.reply(
            f"**Error**\nUsage `{ii}mspam <count> reply to a sticker/gif/photo/video`"
        )
    await event.delete()
