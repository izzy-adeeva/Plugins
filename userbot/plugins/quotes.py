...
@ram_cmd(pattern="love$")
async def love(e):
    txt = choice(LOVESTR)
    await eor(e, txt)


@ram_cmd(pattern="dhoka$")
async def katgya(e):
    txt = choice(DHOKA)
    await eor(e, txt)


@ram_cmd(pattern="metoo$")
async def metoo(e):
    txt = choice(METOOSTR)
    await eor(e, txt)


@ram_cmd(pattern="gdnoon$")
async def noon(e):
    txt = choice(GDNOON)
    await eor(e, txt)


@ram_cmd(pattern="chase$")
async def police(e):
    txt = choice(CHASE_STR)
    await eor(e, txt)


@ram_cmd(pattern="congo$")
async def Sahih(e):
...
@ram_cmd(pattern="gdbye$")
async def bhago(e):
    txt = choice(BYESTR)
    await eor(e, txt)


@ram_cmd(pattern="gdnyt$")
async def night(e):
    txt = choice(GDNIGHT)
    await eor(e, txt)


@ram_cmd(pattern="gdmng$")
async def morning(e):
...
async def quote_search(event):
    ...
    if response is not None:
        await TOD.edit(f"`{response['text']}`")
    else:
        await eod(TOD, "`Sorry Zero results found`")
