...
async def convert_to_image(event, client):
    TOD = await event.get_reply_message()
    if not (
        TOD.gif
        or TOD.audio
        or TOD.voice
    ...
    if not os.path.exists(downloaded_file_name):
        ...
        return
    if TOD and TOD.photo:
        hell_final = downloaded_file_name
    elif TOD.sticker and TOD.sticker.mime_type == "application/x-tgsticker":
    ...
    elif TOD.sticker and TOD.sticker.mime_type == "image/webp":
        ...
        if not os.path.exists(imgpath):
            ...
            return
        hell_final = imgpath
    elif TOD.gif or TOD.video or TOD.video_note:
...
def tgs_to_gif(sticker_path: str, quality: int = 256) -> str:
    semx = os.path.join(dwlpath, "userbottgs.gif")
